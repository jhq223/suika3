 ...
 *
 * These functions have hot blending loops. hal_get_pixel_a() is
 * always a simple bit extraction, but hal_get_pixel_r/g/b() and
 * hal_make_pixel() may contain a branch depending on the active
 * backend pixel order. The accessors below intentionally operate
 * on fixed component slots, not logical RGB channels, so that the
 * compiler can keep this loop branch-free and vectorizable.
 */
        ...
        int alpha)
{
        ...
        for(y = 0; y < height; y++) {
                for(x = 0; x < width; x++) {
                        ...
                        /* Multiply the alpha value and the source pixel value. */
                        src_r = (uint32_t)(src_a * ((float)hal_get_pixel_c1(src_pix) / 255.0f) * 255.0f);
                        src_g = (uint32_t)(src_a * ((float)hal_get_pixel_c2(src_pix) / 255.0f) * 255.0f);
                        src_b = (uint32_t)(src_a * ((float)hal_get_pixel_c3(src_pix) / 255.0f) * 255.0f);
                        ...
                        /* Store to the destination. */
                        *dst_ptr++ = hal_make_pixel_fast(255, add_r, add_g, add_b);
                }
                src_ptr += src_line_inc;
                dst_ptr += dst_line_inc;
        }
        ...
        int alpha)
{
        ...
        for(y = 0; y < height; y++) {
                for(x = 0; x < width; x++) {
                        ...
                        src_r = src_a * 0.5f * (float)hal_get_pixel_c1(src_pix);
                        src_g = src_a * 0.5f * (float)hal_get_pixel_c2(src_pix);
                        src_b = src_a * 0.5f * (float)hal_get_pixel_c3(src_pix);
        ...
        int threshold)
{
        ...
        for (y = 0; y < h; y++) {
                for (x = 0; x < w; x++) {
                        ...
                        /* Store to the destination. */
                        dst_ptr[x] = hal_make_pixel_fast(255,
                                                         (uint32_t)(src_r + dst_r),
                                                         (uint32_t)(src_g + dst_g),
                                                         (uint32_t)(src_b + dst_b));
                }
        ...
        int alpha)
{
        ...
        for (y = 0; y < dst_y_max; y++) {
                ...
                for (x = min_x; x <= max_x; x++) {
                        ...
                        /* Store to the destination. */
                        dst_pixel[y * dw + x] = hal_make_pixel_fast(255,
                                                                    (uint32_t)(src_r + dst_r),
                                                                    (uint32_t)(src_g + dst_g),
                                                                    (uint32_t)(src_b + dst_b));
        ...
        int alpha)
{
        ...
        for (y = 0; y < dst_y_max; y++) {
                ...
                for (x = min_x; x < max_x; x++) {
                        ...
                        /* Multiply the alpha value and the source pixel value. */
                        src_r = (uint32_t)(src_a * (float)hal_get_pixel_c1(src_pix));
                        src_g = (uint32_t)(src_a * (float)hal_get_pixel_c2(src_pix));
                        src_b = (uint32_t)(src_a * (float)hal_get_pixel_c3(src_pix));

                        /* Use the destination pixel value. */
                        dst_r = hal_get_pixel_c1(dst_pix);
                        dst_g = hal_get_pixel_c2(dst_pix);
                        dst_b = hal_get_pixel_c3(dst_pix);
        ...
        int alpha)
{
        ...
        for(y = 0; y < dst_image->height; y++) {
                ...
                for(x = 0; x < dst_image->width; x++) {
                        ...
                        /* Multiply the alpha value and the source pixel value. */
                        src_r = (hal_pixel_t)(src1_a * (float)hal_get_pixel_c1(src1_pix) +
                                              src2_a * (float)hal_get_pixel_c1(src2_pix));
                        src_g = (hal_pixel_t)(src1_a * (float)hal_get_pixel_c2(src1_pix) +
                                              src2_a * (float)hal_get_pixel_c2(src2_pix));
                        src_b = (hal_pixel_t)(src1_a * (float)hal_get_pixel_c3(src1_pix) +
                                              src2_a * (float)hal_get_pixel_c3(src2_pix));

                        /* Multiply the alpha value and the destination pixel value. */
                        dst_r = (hal_pixel_t)(dst_a * (float)(hal_get_pixel_c1(dst_pix)));
                        dst_g = (hal_pixel_t)(dst_a * (float)(hal_get_pixel_c2(dst_pix)));
                        dst_b = (hal_pixel_t)(dst_a * (float)(hal_get_pixel_c3(dst_pix)));

                        /* Store to the destination. */
                        dst_image->pixels[y * dst_image->width + x] =
                                hal_make_pixel_fast(255,
                                                    (uint32_t)(src_r + dst_r),
                                                    (uint32_t)(src_g + dst_g),
                                                    (uint32_t)(src_b + dst_b));
                }
        ...
        int alpha)
{
        ...
        for (y = 0; y < dst_y_max; y++) {
			...
			/* It's faster to skip a pixel with zero-alpha when we can't vectorize ;-) */
                        if (hal_get_pixel_a(src1_pix) == 0 && hal_get_pixel_a(src2_pix) == 0)
				continue;
			...
			dst_pix = dst_pixel[y * dw + x];
                        ...
                        src_r = src1_a * (float)hal_get_pixel_c1(src1_pix) +
				src2_a * (float)hal_get_pixel_c1(src2_pix);
                        src_g = src1_a * (float)hal_get_pixel_c2(src1_pix) +
				src2_a * (float)hal_get_pixel_c2(src2_pix);
                        src_b = src1_a * (float)hal_get_pixel_c3(src1_pix) +
				src2_a * (float)hal_get_pixel_c3(src2_pix);
